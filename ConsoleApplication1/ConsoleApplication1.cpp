...
vector<double> excel(pipe myPipe, ring_buffer_t<vector<vector<double>>>& buffer, int i, vector <double>& data) {
	...
	if (i == 0) {
		...
		for (size_t j = 0; j < current_layer[0].size(); j++) {
			...
			outFile << i * myPipe.get_dt() << "," << (j ) * myPipe.get_dx() << "," << current_layer[0][j] << "," << current_layer[1][j] << "," << current_layer[2][j] << "," << current_layer[2][j]- data[j] << "\n";
...
void excel_2(pipe myPipe, ring_buffer_t<vector<vector<double>>>& buffer, int i, const vector <double>& data) {
	...
	if (i != 0) {
		...
		for (size_t j = 0; j < current_layer[0].size(); j++) {
			...
			outFile << i * myPipe.get_dt() << "," << (j)*myPipe.get_dx() << "," << current_layer[0][j] << "," << current_layer[1][j] << "," << current_layer[2][j] << "," << data[j] << "," << current_layer[2][j] - data[j] << "\n";
